...
int fff(int x, int y, int z) { return x + y - z; }
EOF
...
assert() {
  ...
  echo "$input" | ./target/release/rvrcc -o tmp.s - || exit
  # 编译rvcc产生的汇编文件
...
assert 4 'int a; int main() { a = 1; return sub2(4,3); } int sub2(int x, int y) { int b = 2; return a + b + x-y; }'
...
assert 0 'int main() { return fff(1, 2, 3); }'
assert 4 'int main() { return fff(3, 2, 1); }'
